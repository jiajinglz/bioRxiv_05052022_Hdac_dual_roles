################################################################################
###############Hdac1 Cluster 3 TSA effect AC vs VG##############################
################################################################################
c3_AC_DMSO <- read.delim("~/GDrive files/foxh1 and hdac1/panH3Kac/counts by TSA/hdac1_cluster3/st9_st105_hdac1_only_h3k27ac_peak_1kb.AC_DMSOcounts", header=FALSE)
c3_AC_TSA <- read.delim("~/GDrive files/foxh1 and hdac1/panH3Kac/counts by TSA/hdac1_cluster3/st9_st105_hdac1_only_h3k27ac_peak_1kb.AC_TSAcounts", header=FALSE)
c3_VG_DMSO <- read.delim("~/GDrive files/foxh1 and hdac1/panH3Kac/counts by TSA/hdac1_cluster3/st9_st105_hdac1_only_h3k27ac_peak_1kb.VG_DMSOcounts", header=FALSE)
c3_VG_TSA <- read.delim("~/GDrive files/foxh1 and hdac1/panH3Kac/counts by TSA/hdac1_cluster3/st9_st105_hdac1_only_h3k27ac_peak_1kb.VG_TSAcounts", header=FALSE)

counts_table <- data.frame(matrix(nrow = nrow(c3_AC_DMSO), ncol = 4))
rownames(counts_table) <- paste(c3_AC_DMSO$V1, c3_AC_DMSO$V2, c3_AC_DMSO$V3, sep = "\t")
counts_table[, 1] <- c3_AC_DMSO$V4/3.86  ##divide by scaling factor from spike-in####
counts_table[, 2] <- c3_AC_TSA$V4/1.34   ##divide by scaling factor from spike-in####
counts_table[, 3] <- c3_VG_DMSO$V4/3.95  ##divide by scaling factor from spike-in####
counts_table[, 4] <- c3_VG_TSA$V4/1.13   ##divide by scaling factor from spike-in####
...
library(dplyr)
plot_table <- plot_table %>% mutate(1: nrow(plot_table))
...
ggsave('~/GDrive files/foxh1 and hdac1/panH3Kac/counts by TSA/hdac1_cluster3/panH3Kac_AC_foldChange_along_AC_signal.tiff', g, device = "tiff", dpi = 200)
...
ggsave('~/GDrive files/foxh1 and hdac1/panH3Kac/counts by TSA/hdac1_cluster3/panH3Kac_VG_foldChange_along_VG_signal.tiff.tiff', g, device = "tiff", dpi = 200)
...
ggsave('~/GDrive files/foxh1 and hdac1/panH3Kac/counts by TSA/hdac1_cluster3/vplot_panH3Kac_foldChange_of_spatial_CRMs.tiff', p, device = "tiff", dpi = 200)
